...
export type GovernedConditionAssessment = {
  conditionRef: Ref;
  satisfied: boolean;
  evidenceRefs: Ref[];
};
...
export interface PurposeInstanceRef {
  purpose: "RESTORE_SERVICE";
  situationId: Ref;
  compositionInstanceId: Ref;
  startedFromBasisRef: Ref;
}
...
export interface ActingContextCandidate {
  contextRef: Ref;
  personRef: Ref;
  entityRef?: Ref;
  roleRef: Ref;
  ...
  authorityBasisRef: Ref;
  scopeRef: ScopeRef;
  validity: GovernedConditionAssessment;
  applicability: GovernedConditionAssessment;
  currentness: Currentness;
  integrity: IntegrityAssessment;
  provenance: Provenance;
}

export type ActingContextResolution =
  | { kind: "EXACT_ONE"; candidate: ActingContextCandidate }
  | { kind: "AMBIGUOUS"; candidates: ActingContextCandidate[]; evidenceRefs: Ref[] }
  | { kind: "NONE"; reason: string; evidenceRefs: Ref[] };

export interface GateEnableEvaluationRecord {
  scopeRef: ScopeRef;
  actionId: Ref;
...
export interface LawfulActionProjection {
  scopeRef: ScopeRef;
  actionId: Ref;
  available: boolean;
  blockedReasons: string[];
  requiredContextRefs: Ref[];
  requiredDependencyRefs: Ref[];
  gateEnableRef: Ref;
}
...
export interface ActionCommandEnvelope {
  commandId: Ref;
  actionId: Ref;
  purposeRef: PurposeInstanceRef;
  scopeRef: ScopeRef;
  requestedByActorOrMachineRef: Ref;
  actingContextRef?: Ref;
  boundedMachineAuthorityRef?: Ref;
  expectedInputVersion: number;
  requestTime: IsoInstant;
  payloadRef?: Ref;
  evidenceRefs: Ref[];
}
...
export interface MaterialEffectRecord {
  effectId: Ref;
  commandId?: Ref;
  actionId?: Ref;
  scopeRef: ScopeRef;
