...
const loadCatalog = () => {
  const b64 = fs.readFileSync(new URL("../manifest/frozen-mvm.catalog.json.gz.b64", import.meta.url), "utf8").trim();
...
const variants = [
  {
    ...
    expected: "only governed current intersection is exposed",
    prohibited: "authority-only action leaks past Gate",
  ...
  },
  {
    id: "V08-VP-070",
    vp: "VP-070",
    mode: "HARNESS_TRACE_OBSERVATION",
    ...
    expected: "Ticket identity, responsibility and current control remain anchored to one work context",
    prohibited: "carrier silently substitutes a second Ticket/responsibility identity",
  ...
  },
  {
    id: "V08-VP-142",
    vp: "VP-142",
    mode: "MATERIALIZED_NOT_PRODUCT_EXECUTED",
    ...
    expected: "carrier can orchestrate exact cross-context borrowing challenge under later campaign authority",
    prohibited: "MCR066 smoke awards semantic PASS or mutates Product using foreign authority",
  ...
  },
  {
    ...
    expected: "carrier can orchestrate the no-cross-context-role-borrowing challenge under later campaign authority",
    prohibited: "MCR066 smoke treats eligibility elsewhere as current Ticket authority or awards campaign credit",
...
const evidence = {
  ...
  frozen_trace: {
    scenario: "CS-001",
    obligations: cs001Obligations,
    value_pairs: cs001Vps,
    high_risk_tuples: cs001Hrts,
    mandatory_variants: variants.map((v) => v.id)
  },
  ...
  classification_boundary: {
    harness_failure_prefix: "HARNESS_",
    product_observation: "preserve Product result without repair",
    semantic_ambiguity: "STOP_TO_MCR",
    product_repair_authority: false
  }
};
