...
if (sha256(baseText) !== "662d688ae96c8a161b5ddff607d03ea9ac201ae3636af6be475112485acdb1b7") throw new Error("CATALOG_SPEC_SHA_MISMATCH");
const overrideText = fs.readFileSync(new URL("../manifest/frozen-mvm.catalog.override.spec", import.meta.url), "utf8");
if (sha256(overrideText) !== "1e23b5f1cc89d5aa400a58d31af21e4ad7d5cfb94645ef0abf68636ea22cb904") throw new Error(`CATALOG_OVERRIDE_SHA_MISMATCH:${sha256(overrideText)}`);
...
if(overrideRows.shift()!=="APPTS_MCR066_CATALOG_OVERRIDE_V1") throw new Error("CATALOG_OVERRIDE_HEADER_MISMATCH");
for(const line of overrideRows){const p=line.split("|");if(p[0]!=="OBL_OVERRIDE")throw new Error(`UNKNOWN_OVERRIDE_RECORD:${p[0]}`);if(p[1]!=="MVM-OBL-D07-NFR-002")throw new Error(`UNAUTHORIZED_OVERRIDE_KEY:${p[1]}`);obligation_to_scenarios[p[1]]=splitList(p[2]);}
...
const sectionExpect={product:"5428a2b14072f61257844cd91d27b75ac0cf058e39b56579aee8db816bc7bd4d",sources:"2e4e1bb262a24277a398e13c3bf0396c17db2e550b925fdb91b42be47b925a62",aliases:"909deb20e25d3ba1425dbfcd0f759208bc8b82078716de2996dba96f170cd9f6",obligation_to_scenarios:"7e802116d5aced1db30b5c55220037aa9215ea6466b9142241e91079a34ac24c",vp_to_carrier:"634f763eb566c67f71e675c6b50af23bbbaaf4d1fe3512d077d8692cf5cb48b3",hrt_to_scenarios:"50392eb2c0119423144797aabde843d5e0d6b3f727e7ae1b4a6752f60859f7c9",variant_to_scenario:"8cb50bab417dc1d8dc780c7f3886770e9fa20bbaea507258cac4a3d2c6d39fad",obligation_to_exact_variant:"d666596161a6850659b724ccdd9eeeaa6bb21595bb25e3c3a99c3e76e78d7839"};
for(const[name,expected]of Object.entries(sectionExpect)){const actual=hash(sectionValues[name]);if(actual!==expected)throw new Error(`CATALOG_SECTION_SHA_MISMATCH:${name}:${actual}`)}
...
const result={disposition:"PASS",scope:"MCR066_CARRIER_CATALOG_READINESS_ONLY",no_frozen_campaign_credit:true,catalog_spec_sha256:sha256(baseText),catalog_override_sha256:sha256(overrideText),catalog_semantic_sha256:semanticSha,population:{obligations:344,normalized_aliases:15,value_pairs:527,high_risk_tuples:42,scenarios:35,mandatory_variants:217,exact_f01_bindings:68}};
fs.mkdirSync(new URL("../evidence/",import.meta.url),{recursive:true});fs.writeFileSync(new URL("../evidence/catalog-readiness.json",import.meta.url),JSON.stringify(result,null,2)+"\n");console.log(JSON.stringify(result));