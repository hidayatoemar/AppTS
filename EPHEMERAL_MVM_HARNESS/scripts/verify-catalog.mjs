...
if (sha256(baseText) !== "662d688ae96c8a161b5ddff607d03ea9ac201ae3636af6be475112485acdb1b7") {
  throw new Error("CATALOG_SPEC_SHA_MISMATCH");
}
const overrideText = fs.readFileSync(new URL("../manifest/frozen-mvm.catalog.override.spec", import.meta.url), "utf8");
if (sha256(overrideText) !== "1e23b5f1cc89d5aa400a58d31af21e4ad7d5cfb94645ef0abf68636ea22cb904") {
  throw new Error(`CATALOG_OVERRIDE_SHA_MISMATCH:${sha256(overrideText)}`);
}
...
for (const line of overrideRows) {
  const p = line.split("|");
  if (p[0] !== "OBL_OVERRIDE") throw new Error(`UNKNOWN_OVERRIDE_RECORD:${p[0]}`);
  if (p[1] !== "MVM-OBL-D07-NFR-002") throw new Error(`UNAUTHORIZED_OVERRIDE_KEY:${p[1]}`);
  obligation_to_scenarios[p[1]] = splitList(p[2]);
}

// Exact drift localization against the earlier checksum-bound machine-readable snapshot.
const encodedSnapshot = fs.readFileSync(new URL("../manifest/frozen-mvm.catalog.json.gz.b64", import.meta.url), "utf8").trim();
const compressedSnapshot = Buffer.from(encodedSnapshot, "base64");
let snapshotBuffer;
try {
  snapshotBuffer = zlib.gunzipSync(compressedSnapshot);
} catch (error) {
  if (!(error && error.code === "Z_DATA_ERROR") || compressedSnapshot.length <= 18 || compressedSnapshot[0] !== 0x1f || compressedSnapshot[1] !== 0x8b) throw error;
  snapshotBuffer = zlib.inflateRawSync(compressedSnapshot.subarray(10, -8));
}
const snapshot = JSON.parse(snapshotBuffer.toString("utf8"));
const snapshotVariantMap = snapshot?.crosswalk?.variant_to_scenario;
assert(snapshotVariantMap && typeof snapshotVariantMap === "object", "SNAPSHOT_VARIANT_MAP_MISSING");
const variantKeys = Array.from(new Set([...Object.keys(snapshotVariantMap), ...Object.keys(variant_to_scenario)])).sort();
const variantDiffs = variantKeys
  .filter((id) => snapshotVariantMap[id] !== variant_to_scenario[id])
  .map((id) => ({ id, snapshot: snapshotVariantMap[id] ?? null, clean_spec: variant_to_scenario[id] ?? null }));
console.log(JSON.stringify({
  scope: "MCR066_VARIANT_MAP_EXACT_DRIFT_DIAGNOSTIC",
  snapshot_hash: hash(snapshotVariantMap),
  clean_spec_hash: hash(variant_to_scenario),
  expected_hash: "8cb50bab417dc1d8dc780c7f3886770e9fa20bbaea507258cac4a3d2c6d39fad",
  diff_count: variantDiffs.length,
  diffs: variantDiffs
}));
