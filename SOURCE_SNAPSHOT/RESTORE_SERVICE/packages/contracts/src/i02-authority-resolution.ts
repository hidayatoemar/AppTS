...
export interface AuthorityResolutionPayload {
  readonly authority_result_id: ContractId; readonly ticket_id: ContractId; readonly domain_id: ContractId;
...
const KEYS = new Set(["authority_result_id", "ticket_id", "domain_id", "context_ref", "assignment_snapshot_refs", "responsibility_id", "responsible_assignment_ref", "authority_actions", "sod_decision_ref", "result_status_ref", "currentness_ref", "effective_from", "effective_to", "refresh_lineage_ref", "no_valid_refresh_ref", "predecessor_authority_result_ref", "supersedes_authority_result_ref"]);
...
function validatePayload(payload: unknown): readonly ValidationIssue[] {
  ...
  for (const key of ["authority_result_id", "ticket_id", "domain_id", "result_status_ref", "currentness_ref"]) requireNonEmptyString(payload, key, issues, "payload.");
