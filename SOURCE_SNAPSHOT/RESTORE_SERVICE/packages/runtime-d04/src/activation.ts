...
export function activateRuntime(existing: RuntimeAggregate | undefined, message: TicketActivationMessage, validation: ValidationResult): ActivationResult {
  ...
  if (existing) return existing.activationId === message.payload.activation_id && existing.ticketId === message.payload.ticket_id ? { status: "REPLAY", aggregate: existing } : { status: "HELD_CONFLICT", reason: "CONFLICTING_ACTIVATION" };
  return { status: "ACTIVATED", aggregate: Object.freeze({ ticketId: message.payload.ticket_id, state: "ACCEPTED", aggregateVersion: 0, purposeBindingId: message.payload.purpose_binding_id, purposeIdentity: message.payload.purpose_identity, purposeVersion: message.payload.purpose_version, packageIdentity: message.payload.package_identity, packageVersion: message.payload.package_version, activationId: message.payload.activation_id }) };
}
