...
test("D01 forms I01 activation only after every admission predicate is satisfied", () => {
  ...
  const formed = formTicket(assessment, { formationId: "f-1", ticketId: "t-1", domainId: "d-1", intakeDecisionId: "decision-1", responsibleAssignmentRef: "assignment-1", formationEvidenceSetRef: "evidence-set-1", aggregateVersion: 1, effectiveAt: "2026-08-11T00:00:00Z", binding });
...
test("D01 fails closed for missing or stale admission facts", () => {
  ...
  assert.throws(() => formTicket(stale, { formationId: "f", ticketId: "t", domainId: "d", intakeDecisionId: "i", responsibleAssignmentRef: "r", formationEvidenceSetRef: "e", aggregateVersion: 1, effectiveAt: "2026-08-11T00:00:00Z", binding }), /ADMISSION_NOT_ACCEPTABLE/);
});

test("D01 never reopens CLOSED tickets", () => {
  assert.throws(() => transitionTicket({ ticketId: "t", state: "CLOSED", relations: [] }, "ACTIVE"), /CLOSED_TICKET_CANNOT_REOPEN/);
});
