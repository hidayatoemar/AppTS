...
{
  if [[ "$INDEP_HOST" =~ ^[0-9]+(\.[0-9]+){3}$ ]]; then echo 'target_binding_type=ip_literal'; else echo 'target_binding_type=hostname'; fi
...
"${SSH[@]}" bash -s <<'REMOTE' | tee "$E/02-server-listener-firewall.txt"
set -euo pipefail
echo '== listeners =='
...
"${SSH[@]}" bash -s <<'REMOTE' | tee "$E/03-runtime-day2-health.txt"
set -euo pipefail
...
"${SSH[@]}" bash -s <<'REMOTE' | tee "$E/04-edge-hostname-tls.txt"
set -euo pipefail
...
if [ -d /etc/letsencrypt/live ]; then
  find /etc/letsencrypt/live -mindepth 1 -maxdepth 1 -type d -printf 'letsencrypt_name=%f\n' 2>/dev/null | sort
else
  echo 'letsencrypt_live=absent'
fi
echo "certbot=$(command -v certbot >/dev/null 2>&1 && echo installed || echo absent)"
REMOTE

"${SSH[@]}" bash -s <<'REMOTE' | tee "$E/05-browser-proxy-readiness.txt"
set -euo pipefail
...
set_cookie_count=$(grep -ic '^set-cookie:' "$hdr" || true)
location_count=$(grep -ic '^location:' "$hdr" || true)
absolute_loopback_url_count=$(grep -Eoc 'http://(127\.0\.0\.1|localhost)(:[0-9]+)?' "$tmp" || true)
relative_href_src_count=$(grep -Eoc '(href|src)="/' "$tmp" || true)
echo "root_http_status=$status"
echo "set_cookie_count=$set_cookie_count"
echo "location_count=$location_count"
echo "absolute_loopback_url_count=$absolute_loopback_url_count"
echo "relative_href_src_count=$relative_href_src_count"
REMOTE
...
elif [ "$TARGET_TYPE" = 'ip_literal' ] && [ -z "$HOST_CLUES" ]; then
  DISPOSITION=STOP
