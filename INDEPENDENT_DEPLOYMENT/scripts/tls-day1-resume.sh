...
# Fail closed on the exact interrupted MCR-to-DT-014 state.
...
assert d['holder_ref'] and d['role_assignment_ref'] and d['action_contract_ref']=='INT-RUN-TD-05', d
...
# Prove failed prior ACTIVATE attempts did not cross validation into durable state.
"${SSH_BASE[@]}" "$REMOTE_HOST" \
  "sudo docker compose -f '$COMPOSE_FILE' --env-file '$COMPOSE_ENV' exec -T postgres psql -X -U appts_admin -d appts_dep001_independent -Atqc \"SELECT 'ticket_action_intents='||count(*) FROM appts.action_intent WHERE ticket_id='${TICKET_ID}'::uuid; SELECT 'effect_requests='||count(*) FROM appts.lifecycle_effect_request WHERE ticket_id='${TICKET_ID}'::uuid; SELECT 'transitions='||count(*) FROM appts.runtime_state_transition WHERE ticket_id='${TICKET_ID}'::uuid;\"" \
  | tee "$EVIDENCE_DIR/pre-action-durable-readback.txt"
grep -Fxq 'ticket_action_intents=0' "$EVIDENCE_DIR/pre-action-durable-readback.txt"
grep -Fxq 'effect_requests=0' "$EVIDENCE_DIR/pre-action-durable-readback.txt"
grep -Fxq 'transitions=0' "$EVIDENCE_DIR/pre-action-durable-readback.txt"

# Execute the accepted INT-RUN-TD-05 action through the deployed HTTP boundary.
# The accepted contract permits only 1-3 fractional-second digits; Python's
# default isoformat emits 6. Therefore DT serializes the same UTC instant at
# millisecond precision, matching the frozen contract without changing Product source.
python3 - "$PORT" "$EVIDENCE_DIR/current-before.json" "$EVIDENCE_DIR/action-result.json" "$EVIDENCE_DIR/action-http-error.json" <<'PY'
import datetime, json, sys, uuid, urllib.error, urllib.request
port, before_path, out, errout = sys.argv[1:]
base=f"http://127.0.0.1:{port}/api/v1/ui"
...
intent_time=datetime.datetime.now(datetime.timezone.utc).isoformat(timespec='milliseconds').replace('+00:00','Z')
...
# Independent durable current-server readback after first action + identical replay.
"${SSH_BASE[@]}" "$REMOTE_HOST" \
...
action_id=${ACTION_ID}
health_ready=PASS
pre_action_no_durable_action_state=PASS
current_before=ACCEPTED_v0_ACTIVATE_PRESENTED
intent_timestamp_contract_format=MILLISECOND_RFC3339_PASS
