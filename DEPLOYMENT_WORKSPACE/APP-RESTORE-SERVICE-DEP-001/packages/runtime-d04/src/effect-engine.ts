...
const attribution = (command: EffectCommand): Readonly<Partial<Pick<DurableEffectResult, "entityRef" | "actorHolderRef" | "actingRoleInstanceRef" | "actingAssignmentRef" | "authorityBasisRef">>> => Object.freeze({ ...(command.entityRef ? { entityRef: command.entityRef } : {}), ...(command.actorHolderRef ? { actorHolderRef: command.actorHolderRef } : {}), ...(command.actingRoleInstanceRef ? { actingRoleInstanceRef: command.actingRoleInstanceRef } : {}), ...(command.actingAssignmentRef ? { actingAssignmentRef: command.actingAssignmentRef } : {}), ...(command.authorityBasisRef ? { authorityBasisRef: command.authorityBasisRef } : {}) });
const noEffect = (command: EffectCommand, aggregate: RuntimeAggregate, reason: string): DurableEffectResult => Object.freeze({ commandId: command.commandId, payloadHash: command.payloadHash, ...attribution(command), disposition: "NO_EFFECT", reason, aggregateVersion: aggregate.aggregateVersion, state: aggregate.state, auditRef: `audit:${command.commandId}` });
const completeAttribution = (command: EffectCommand): command is EffectCommand & Required<Pick<EffectCommand, "entityRef" | "actorHolderRef" | "actingRoleInstanceRef" | "actingAssignmentRef" | "authorityBasisRef">> => [command.entityRef, command.actorHolderRef, command.actingRoleInstanceRef, command.actingAssignmentRef, command.authorityBasisRef].every((value) => typeof value === "string" && value.length > 0);
...
export async function executeLifecycleEffect(store: RuntimeEffectStore, command: EffectCommand): Promise<DurableEffectResult> {
  ...
  if (!aggregate.entityRef || command.entityRef !== aggregate.entityRef) return noEffect(command, aggregate, "WRONG_ENTITY_NO_EFFECT");
