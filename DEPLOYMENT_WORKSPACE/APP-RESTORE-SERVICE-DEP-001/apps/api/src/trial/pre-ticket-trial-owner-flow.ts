...
async function realizeTrialSourceBinding(pool: Pool, ids: MechanicalIds): Promise<string> {
...
async function inTransaction<Result>(pool: Pool, work: (client: PoolClient) => Promise<Result>): Promise<Result> {
...
async function audit(client: PoolClient, event: string, subjectId: string, submission: PreTicketSubmission, ids: MechanicalIds, disclosure: string): Promise<void> {
  await client.query(
...
async function ptx01(pool: Pool, submission: PreTicketSubmission, ids: MechanicalIds, disclosure: string): Promise<Ptx01Result> {
...
async function ptx02And03(pool: Pool, submission: PreTicketSubmission, ids: MechanicalIds, disclosure: string, caseId: string, evaluation: PreTicketAdmissionEvaluation): Promise<{ readonly assessmentId: string; readonly decisionId: string }> {
...
export function createTrialPreTicketOwnerFlow(pool: Pool): (payload: unknown) => Promise<TrialOwnerFlowResult> {
...
export function createTrialPreTicketIntentDispatcher(pool: Pool): UiIntentDispatcher {
