...
export function registerTlsDay3AuthRoutes(app:FastifyInstance,auth:TlsDay3TrialAuth):void{
  app.post(`${AUTH_PREFIX}/login`,async(request,reply)=>{try{const body=request.body&&typeof request.body==="object"?request.body as Readonly<Record<string,unknown>>:{};const session=auth.login(body["alias"],body["credential"],request,reply);return reply.header("Cache-Control","no-store").send(publicSession(session));}catch(error){return failure(reply,error);}});
  app.get(`${AUTH_PREFIX}/session`,async(request,reply)=>{try{return reply.header("Cache-Control","no-store").send(publicSession(auth.require(request)));}catch(error){return failure(reply,error);}});
